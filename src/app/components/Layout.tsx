...
import { Package, ShoppingBag, Settings2 } from 'lucide-react';
...
export function Layout() {
  ...
  return (
    <div className="min-h-screen bg-violet-50/40 pb-20 font-sans selection:bg-violet-100">
      <header className="bg-white/80 backdrop-blur-xl sticky top-0 z-20 border-b border-violet-100/60 shadow-sm shadow-violet-100/20">
        <div className="px-5 py-4 max-w-lg mx-auto flex items-center justify-center">
          <h1 className="text-[17px] font-bold tracking-tight text-slate-800">
            {location.pathname === '/shopping-list' ? 'Shopping List' : location.pathname === '/manage' ? 'Manage Pantry' : 'Inventory'}
          </h1>
        </div>
      </header>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-violet-100/60 shadow-[0_-4px_20px_-4px_rgba(139,92,246,0.05)] pb-safe z-20">
        <div className="flex justify-around items-center h-[72px] max-w-lg mx-auto px-2">
          <Link
            ...
            className={`flex flex-col items-center justify-center w-full h-full gap-1.5 transition-all ${
              location.pathname === '/pantry'
                ? 'text-violet-600 scale-[1.02]'
                : 'text-slate-400 hover:text-slate-600 hover:bg-violet-50/50 rounded-2xl my-1'
            }`}
          >
            <Package size={24} strokeWidth={location.pathname === '/pantry' ? 2.5 : 2} />
          ...
          >
            <div className="relative">
              <ShoppingBag size={24} strokeWidth={location.pathname === '/shopping-list' ? 2.5 : 2} />
              {shoppingCount > 0 && (
          ...
          >
            <Settings2 size={24} strokeWidth={location.pathname === '/manage' ? 2.5 : 2} />
            <span className="text-[11px] font-bold tracking-wide">Manage</span>
          </Link>
        </div>
      </nav>
    </div>
